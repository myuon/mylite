...
type Executor struct {
	...
	// warningActualCount tracks the total number of warnings generated by the current
	// statement, including those dropped due to max_error_count. MySQL's @@warning_count
	// reports this total rather than the stored count.
	warningActualCount int64
...
func (e *Executor) addWarning(level string, code int, message string) {
	// Always count the warning for @@warning_count, even if it won't be stored.
	e.warningActualCount++
...
func extractCharLength(colType string) int {
	...
	// CHAR/BINARY without explicit length default to 1 in MySQL.
	if lower == "char" {
		return 1
	}
	if lower == "binary" {
...
func (e *Executor) validateIndexHints(from []sqlparser.TableExpr) error {
	for _, te := range from {
		...
		case *sqlparser.AliasedTableExpr:
			...
			// Invisible indexes are not visible to index hints (FORCE/USE/IGNORE INDEX)
			// unless the use_invisible_indexes optimizer switch is enabled.
			useInvisibleIndexes := false
			if sw, ok := e.getSysVar("optimizer_switch"); ok {
				for _, part := range strings.Split(sw, ",") {
					part = strings.TrimSpace(part)
					if strings.EqualFold(part, "use_invisible_indexes=on") {
			...
			for _, idx := range def.Indexes {
				if idx.Invisible && !useInvisibleIndexes {
					continue // invisible indexes are not accessible via index hints
				}
