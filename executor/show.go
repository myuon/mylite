...
func (e *Executor) execShow(stmt *sqlparser.Show, query string) (*Result, error) {
	...
	if strings.HasPrefix(upper, "SHOW CREATE DATABASE") || strings.HasPrefix(upper, "SHOW CREATE SCHEMA") {
		...
		if len(parts) >= 4 {
			...
			// Preprocess rewrites "SHOW CREATE DATABASE IF NOT EXISTS db" to
			// "SHOW CREATE DATABASE db" and sets e.showCreateDBIfNotExists.
			dbName := parts[3]
			ifNotExists := e.showCreateDBIfNotExists
			e.showCreateDBIfNotExists = false // reset after consuming
...
func (e *Executor) execShowGrants(query string) (*Result, error) {
	...
	if strings.EqualFold(grantUser, "root") {
		...
		// 1. Static privileges including CREATE ROLE, DROP ROLE
		// 2. Dynamic privileges (APPLICATION_PASSWORD_ADMIN, etc.)
		// 3. GRANT PROXY
		grantStrs = []string{
			fmt.Sprintf("GRANT SELECT, INSERT, UPDATE, DELETE, CREATE, DROP, RELOAD, SHUTDOWN, PROCESS, FILE, REFERENCES, INDEX, ALTER, SHOW DATABASES, SUPER, CREATE TEMPORARY TABLES, LOCK TABLES, EXECUTE, REPLICATION SLAVE, REPLICATION CLIENT, CREATE VIEW, SHOW VIEW, CREATE ROUTINE, ALTER ROUTINE, CREATE USER, EVENT, TRIGGER, CREATE TABLESPACE, CREATE ROLE, DROP ROLE ON *.* TO `%s`@`%s` WITH GRANT OPTION", grantUser, grantHost),
			fmt.Sprintf("GRANT APPLICATION_PASSWORD_ADMIN,AUDIT_ADMIN,BACKUP_ADMIN,BINLOG_ADMIN,BINLOG_ENCRYPTION_ADMIN,CLONE_ADMIN,CONNECTION_ADMIN,ENCRYPTION_KEY_ADMIN,GROUP_REPLICATION_ADMIN,INNODB_REDO_LOG_ARCHIVE,PERSIST_RO_VARIABLES_ADMIN,REPLICATION_SLAVE_ADMIN,RESOURCE_GROUP_ADMIN,RESOURCE_GROUP_USER,ROLE_ADMIN,SERVICE_CONNECTION_ADMIN,SESSION_VARIABLES_ADMIN,SET_USER_ID,SYSTEM_USER,SYSTEM_VARIABLES_ADMIN,TABLE_ENCRYPTION_ADMIN,XA_RECOVER_ADMIN ON *.* TO `%s`@`%s` WITH GRANT OPTION", grantUser, grantHost),
