...
func (e *Executor) execInsert(stmt *sqlparser.Insert) (*Result, error) {
	...
	// directInsertIDVal records the exact value specified by SET @@INSERT_ID so that
	// the first auto-generated row in this statement uses it directly (bypassing the
	// auto_increment_increment/offset alignment). MySQL inserts the exact INSERT_ID
	// value for the first auto row, then resumes normal aligned generation thereafter.
	var directInsertIDVal int64
	if e.nextInsertID > 0 {
		directInsertIDVal = e.nextInsertID
		// Only advance the counter if insert_id exceeds the current counter.
		// Do NOT lower the counter: if the table's auto_increment is already
		// higher than nextInsertID, the explicit row should be inserted at
		// nextInsertID but subsequent rows must resume from the existing counter.
		if cur := tbl.AutoIncrementValue(); e.nextInsertID > cur {
			tbl.AutoIncrement.Store(e.nextInsertID - 1)
		}
		tbl.AIExplicitlySet = true
		e.nextInsertID = 0
		// Sync the session variable so SELECT @@session.insert_id returns 0 after INSERT.
		e.sessionScopeVars["insert_id"] = "0"
	}
	...
	insertSelectFastPath := func(selResult *Result) (*Result, error) {
		...
		if autoColName == "" && len(bulkRows) > 0 {
			...
			for _, row := range bulkRows {
				...
				// Track rows modified for INFORMATION_SCHEMA.INNODB_TRX.
				// This applies for both explicit (BEGIN) and implicit (autocommit=0) transactions.
				if e.txnActiveSet != nil {
					e.txnActiveSet.IncrRowsModified(e.connectionID, 1)
				}
				lastInsertID = id
	...
	for rowIdx, valTuple := range rows {
		...
		// Compute aligned next AI value when auto_increment_increment/offset are non-default.
		// This ensures the storage engine sees the same aligned value MySQL would
		// generate. Without alignment, an explicit value such as 2 inserted in the
		// same statement could collide with a NULL-generated value.
		if autoGeneratedThisRow && autoColName != "" {
			// When SET @@INSERT_ID=N was used, apply it directly to the first auto-generated
			// row without alignment. MySQL uses the exact INSERT_ID value for the first row,
			// then resumes normal aligned generation for subsequent rows.
			// Exception: nullable AUTO_INCREMENT columns with explicit NULL inserts keep NULL
			// when AIExplicitlySet is true (SET INSERT_ID forces the counter but doesn't
			// override nullable NULL values – they stay NULL per MySQL behavior).
			directApplied := false
			if directInsertIDVal > 0 {
				// Check if the auto column is nullable and original value was NULL
				autoColNullable := false
				origValIsNil := false
				for _, c := range tbl.Def.Columns {
					if strings.EqualFold(c.Name, autoColName) {
						autoColNullable = c.Nullable
						break
					}
				}
				if v, exists := row[autoColName]; exists && v == nil {
				...
				if !(autoColNullable && origValIsNil) {
					row[autoColName] = directInsertIDVal
					// Only advance the counter if the direct value exceeds the current
					// counter, so the table's auto_increment doesn't go backwards when
					// insert_id is set to a value lower than the current counter.
					if cur := tbl.AutoIncrementValue(); directInsertIDVal >= cur {
						tbl.AutoIncrement.Store(directInsertIDVal)
					}
					autoGeneratedThisRow = false
					directInsertIDVal = 0 // consumed; subsequent rows use normal alignment
					directApplied = true
				}
			}
			if !directApplied {
			...
			if aiIncrement > 1 || aiOffset != 1 {
				...
				// Strict-mode range check on the aligned value.
				if e.isStrictMode() {
					for _, col := range tbl.Def.Columns {
				...
				// Pre-fill the row with the aligned AI value so storage Insert
				// treats it as an explicit value (and bumps the counter accordingly).
				if autoGeneratedThisRow {
					row[autoColName] = nextAI
					autoGeneratedThisRow = false
				}
			}
			} // end else (directInsertIDVal == 0)
		} else if autoGeneratedThisRow && e.isStrictMode() && autoColName != "" {
			// Default increment/offset path: still range-check the next value.
			cur := tbl.AutoIncrementValue()
			nextAI := cur + 1
			for _, col := range tbl.Def.Columns {
				if col.AutoIncrement && strings.EqualFold(col.Name, autoColName) {
